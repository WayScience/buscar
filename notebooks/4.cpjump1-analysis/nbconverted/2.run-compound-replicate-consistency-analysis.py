...
def run_replicate_consistency_analysis(
    ...
    if checked_trts is not None:
        treatments = [
            ...
            if t in checked_trts
        ]
    else:
        print("DEBUGG: filtering to only unprocessed treatments")
        treatments = profiles["Metadata_pert_iname"].unique().to_list()
        print(f"DEBUGG: treatments to analyze: {len(treatments)}")
    ...
    for treatment in treatment_pbar:
        ...
        for iteration in tqdm(
            ...
            for ref_plate in tqdm(
                ...
                # Select a single treament from each plate (execept the ref)
                if random_trt_comparison:
                    ...
                    # randomly a single treatment from each plate
                    ...
                    for idx, sel_plate in enumerate(plates_to_compare):
                        ...
                        sel_plates_trt_to_compare.append(
                            profiles.filter(
                                ...
                                & (pl.col("Metadata_pert_iname") == random_trts[idx])
                            )
                        )
                    ...
                    plate_to_test = pl.concat(
                        [
                            ref_negcon,
                            profiles.filter(pl.col("Metadata_pert_iname") == treatment),
                        ]
                    )
                ...
                for row in scores_df.iter_rows(named=True):
                    ...
                    if save_dir is not None:
                        save_path = (
                            save_dir
                            / f"{cell_type}_{'shuffled' if random_trt_comparison else 'original'}_replicate-tracking.jsonl"
                        ).resolve()
...
u2os_rnd_trt_results_df.write_parquet(
    outdir / "u2os_random_trt_replicate_consistency_results.parquet"
)
...
a549_rnd_trt_results_df.write_parquet(
    outdir / "a549_random_trt_replicate_consistency_results.parquet"
)
